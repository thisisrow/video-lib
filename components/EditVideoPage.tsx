...
/**
 * A page that allows the user to edit the description of a video or create a new one.
...
export const EditVideoPage: React.FC<EditVideoPageProps> = ({
  ...
  // If videoUrl is empty, we treat this as creating a new video from scratch
  const isNewVideo = !video.videoUrl;
      ...
      <div className="w-full max-w-2xl bg-gray-800 p-6 md:p-8 rounded-lg shadow-2xl">
        <header className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-1">
            {isNewVideo ? 'Create New Video' : 'Edit Video'}
          </h1>
        </header>
        ...
        <main>
          <div className="mb-6">
            <label
              ...
              {isNewVideo ? 'Describe the video you want to generate' : 'Video text prompt'}
            </label>
            <textarea
              id="description"
              rows={10}
              ...
              placeholder={isNewVideo ? "A cyberpunk city with neon lights..." : ""}
              aria-label={isNewVideo ? "Description for new video" : "Edit description for the video"}
            />
          </div>
        </main>
        ...
        <footer className="flex justify-end gap-4">
          ...
          <button
            ...
            className="px-6 py-2 rounded-lg bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold transition-colors">
